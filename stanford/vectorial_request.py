...
STANDFORD_NUMBER_OF_DOCS = 98997
...
def vectorial_search_cacm(query, number_of_documents):
    ...
    w = {-1: {}}
    s = STANDFORD_NUMBER_OF_DOCS * [0]
    for termID in parsed_query:
        nb = parsed_query.count(termID)
        w[-1][termID] = (1 + math.log10(nb)) * math.log10(STANDFORD_NUMBER_OF_DOCS / index[str(termID)][0])
        ...
        for tup in postings:
            ...
            if freq == 0:
                ...
                w[docID][termID] = (1 + math.log10(freq)) * math.log10(STANDFORD_NUMBER_OF_DOCS / index[str(termID)][0])
...
def vectorial_search_cacm_normalized(query, number_of_documents):
    ...
    for j in range(STANDFORD_NUMBER_OF_DOCS):
        if s[j] != 0:
...
with open("../standford_index.json", 'r') as index_file:
